...
function Drawingarea({ selectedShape, ColorShape, setShapes, shapes, stageRef }) {
  ...
  const renderShape = (shape, key) => {
    ...
    const handleDragEnd = (e) => {
      ...
      setShapes((prevShapes) => prevShapes.map((s, index) => (index === key ? { ...s, x, y } : s)));
      setIsDrawing(true);
    };
      ...
      case "Star":
        return (
          <Star
            ...
            stroke="black"
            strokeWidth={borderSize}
            innerRadius={Math.abs(width / 4)}
            outerRadius={Math.abs(width / 2)}
            fill={color}
            onClick={handleClick}
            onDragStart={handleDragStart}
            onDragEnd={handleDragEnd}
      ...
      <div className="bg-white sm:w-[40%] md:w-[40%] lg:w-[40%] h-[8%] w-[50%] rounded-2xl flex justify-around items-center">
        ...
        <FaEraser
          className={`cursor-pointer text-sm sm:text-xl md:text-2xl lg:text-3xl ${
            isEraserActive ? "bg-gray-300 rounded-lg" : ""
          }`}
          onClick={() => {
        ...
        {/* Pencil Sizes */}
        <GiPencil
          className={`text-sm sm:text-xl md:text-2xl lg:text-2xl w-10 ${
            borderSize === 3 && isEraserActive == false ? "bg-gray-300 rounded-lg" : ""
          }`}
          onClick={() => {
            setborderSize(3);
            if (borderSize == 3) {
        ...
        />
        <GiPencil
          className={`text-lg sm:text-2xl md:text-3xl lg:text-3xl w-10 ${
            borderSize === 4 && isEraserActive == false ? "bg-gray-300 rounded-lg" : ""
          }`}
          onClick={() => {
            setborderSize(4);
            if (borderSize == 4) {
        ...
        />
        <GiPencil
          className={`text-2xl sm:text-3xl md:text-4xl lg:text-4xl w-10 ${
            borderSize === 7 && isEraserActive == false ? "bg-gray-300 rounded-lg" : ""
          }`}
          onClick={() => {
            setborderSize(7);
            if (borderSize == 7) {
