...
const ArchivePage = () => {
  ...
  const [filteredData, setFilteredData] = useState(draws);
  ...
  const applyNameFilter = () => {
    if (valueNameFilter.trim() === "") {
      setFilteredData(draws); // Reset to full data if filter is empty
    } else {
      const filtered = draws.filter((file) =>
  ...
  const applyCreatedByFilter = () => {
    if (valueCreatedByFilter.trim() === "") {
      ...
      const filtered = draws.filter((file) => {
        console.log(file.child.name + "compared to" + valueCreatedByFilter);
        if (file.child.name.toLowerCase().includes(valueCreatedByFilter.toLowerCase())) {
          console.log("true");
        }
        return file.child.name.toLowerCase().includes(valueCreatedByFilter.toLowerCase());
      });
      setFilteredData(filtered);
    }
        ...
        {showFilterMenu && (
          <div className="absolute right-0 mt-[20%] bg-white shadow-lg rounded-lg p-2 w-[15%]">
            ...
            <div className="flex justify-between">
              ...
              <input
                type="checkbox"
                checked={isCheckedCreatedBy}
                onChange={handleCheckboxCreatedByChangeChange}
              />
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-4 rounded-lg lg:w-[30%] w-[90%] shadow-lg">
            ...
            <div className="flex justify-end space-x-2">
              <button className="px-4 py-2 bg-gray-300 rounded" onClick={() => setShowCreatedByDialog(false)}>
                Cancel
              </button>
              <button className="px-4 py-2 bg-blue-500 text-white rounded" onClick={applyCreatedByFilter}>
                Apply
              </button>
            </div>
          </div>
          ...
          <tbody>
            {filteredData?.map((file, index) => (
              <tr key={index} className="hover:bg-gray-50 transition-colors border-b border-gray-200">
                ...
                <td className="p-4 text-gray-600">{file.last_modified}</td>
                ...
                <td className="p-4 flex items-center space-x-2">
                  ...
                  />
                  <span className="text-gray-700">{file.child.name}</span>
                </td>
              </tr>
            ))}
          </tbody>
